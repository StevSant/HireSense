...
class RecruiteeNormalizer:
    def normalize(self, raw: RawJobListing) -> dict[str, Any]:
        ...
        salary_range = (
            f"{currency} {salary_min}-{salary_max}".strip() if salary_min and salary_max else None
        )
        is_remote = bool(d.get("remote"))
        country = d.get("country_code", "")
        place = d.get("location") or ", ".join(p for p in (d.get("city", ""), country) if p)
        if is_remote:
        ...
        description = strip_html(
            "\n".join(p for p in (d.get("description", ""), d.get("requirements", "")) if p)
        )
        tags = d.get("tags") or []
