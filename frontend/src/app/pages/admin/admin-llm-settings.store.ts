...
/**
 * State and orchestration for the admin LLM settings screen.
 *
 * Provided by AdminLLMSettingsComponent, so the page shares one instance that
 * is discarded on leave — an abandoned override draft never survives to the
 * next visit.
 *
 * Two concerns live here: the global provider/model form (including its
 * test-before-save handshake), and the per-feature override draft. They share
 * a store because both write `features` through the same loadFeatures()
 * refresh, and both report into the same `loading` flag.
 */
@Injectable()
export class AdminLLMSettingsStore {
  private api = inject(AdminLLMSettingsService);
  ...
  resetOverride(feature: FeatureView): void {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (err: HttpErrorResponse) =>
          this.overrideError.set(err?.error?.detail ?? 'Reset failed'),
      });
  }
}
