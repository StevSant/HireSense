...
# target, matching hiresense.shared.config. alembic.ini ships without credentials; its
# `sqlalchemy.url` is only a fallback for tooling that sets it explicitly. The
# app strips `+asyncpg` for its sync engine (see composition.shared_infra), but
