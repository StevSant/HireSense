...
import { ComboboxOption } from '@core/components/combobox';
...
import { SortDirection, createSortState } from '@core/utils/sort-state';
import { sortItems } from '@core/utils/sort-items';
import { environment } from '@env/environment';

/**
 * State and orchestration for the Outreach page.
 *
 * Provided by OutreachComponent rather than the route, so the page shares one
 * instance that is discarded on leave. That lifetime is deliberate for
 * `seededMessageKeys` below — it must reset per visit — while the generation
 * run itself lives in the root-scoped LlmRunnerService precisely so it does
 * NOT die with this store.
 *
 * Six concerns live here, in this order below: the target picker, compose +
 * generate, recording an event, the timeline, contact suggestions, and
 * follow-up nudges. They share one store because they all key off the single
...
export class OutreachStore {
  ...
  // `message` on this store instance — either by a hydration read in
  ...
  /**
   * The page's bootstrap requests.
   *
   * Driven from the component's ngOnInit rather than the constructor so the
   * request order stays exactly as it was: applications first (its response
   * is what preselects from the query param), then nudges.
   */
  init(): void {
  ...
  setEventSort(field: 'created', dir: SortDirection): void {
    this.eventSort.set(field, dir);
  }

  setKindFilter(kind: OutreachEventKind | ''): void {
