...
def test_resolves_from_global_row_with_decrypted_key() -> None:
    ...
    row = _FakeSettingsRow(provider="openai", model="gpt-4o-mini", api_key_encrypted=encrypted, extra_params={})
...
def test_falls_back_to_env_when_global_has_no_key() -> None:
    row = _FakeSettingsRow(provider="openai", model="gpt-4o-mini", api_key_encrypted=None, extra_params={})
...
def test_override_provider_only_inherits_model() -> None:
    row = _FakeSettingsRow(provider="anthropic", model="claude-sonnet-4-6", api_key_encrypted=None, extra_params={})
    overrides = {"cv_parser": _FakeOverrideRow(
        feature_key="cv_parser", provider="openai", model=None, extra_params={},
    )}
...
def test_override_model_only_inherits_provider() -> None:
    row = _FakeSettingsRow(provider="anthropic", model="claude-sonnet-4-6", api_key_encrypted=None, extra_params={})
    overrides = {"cv_parser": _FakeOverrideRow(
        feature_key="cv_parser", provider=None, model="claude-haiku-4-5", extra_params={},
    )}
    svc = _service(row, overrides, APIKeyCipher(""))
    config = svc.resolve("cv_parser")
...
def test_extra_params_merge_override_wins() -> None:
    row = _FakeSettingsRow(
        provider="anthropic", model="claude-sonnet-4-6", api_key_encrypted=None,
        extra_params={"temperature": 0.2, "max_tokens": 1024},
    )
    overrides = {"cv_parser": _FakeOverrideRow(
        feature_key="cv_parser", provider=None, model=None,
...
def test_other_feature_unaffected_by_override() -> None:
    row = _FakeSettingsRow(provider="anthropic", model="claude-sonnet-4-6", api_key_encrypted=None, extra_params={})
    overrides = {"cv_parser": _FakeOverrideRow(
        feature_key="cv_parser", provider="openai", model="gpt-4o-mini", extra_params={},
    )}
    svc = _service(row, overrides, APIKeyCipher(""))
...
def test_invalidate_clears_cache() -> None:
    ...
    state["row"] = _FakeSettingsRow(
        provider="openai", model="new-model", api_key_encrypted=None, extra_params={},
    )
